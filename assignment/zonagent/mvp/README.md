...
- ✅ 3가지 플랫폼 (Granicus, CivicEngage, CivicClerk)
- ✅ Backfill 모드 (과거 데이터 수집)
- ✅ SQLite 데이터베이스
- ✅ 유연한 기술 스택 (httpx + BeautifulSoup + Playwright)
...
│   │   ├── playwright_scraper.py  # PlaywrightScraper 베이스
│   │   ├── cherokee.py         # CherokeeScraper (Granicus)
│   │   ├── marietta.py         # MariettaScraper (CivicEngage)
│   │   ├── alpharetta.py       # AlpharettaScraper (CivicClerk)
│   │   └── holly_springs.py    # HollySpringScraper (CivicClerk)
│   ├── utils.py                # 유틸리티 함수
│   ├── config.py               # 설정 관리
│   └── main.py                 # CLI 진입점
├── tests/                      # 테스트 (선택)
...
# Playwright 브라우저 설치 (Alpharetta, Holly Springs 필요)
...
# Alpharetta 데이터 수집 (Playwright 사용)
python -m src.main backfill --jurisdiction alpharetta --limit 10

# Holly Springs 데이터 수집 (Playwright 사용)
...
### Phase 3: Alpharetta + Holly Springs (완료) ✅

- [x] Playwright 통합
- [x] PlaywrightScraper 베이스 클래스
- [x] AlpharettaScraper 구현 (CivicClerk SPA)
- [x] HollySpringScraper 구현 (코드 재사용)
- [x] 4개 지자체 CLI 통합
- [x] 유연한 HTML 구조 지원
...
- Continuous 모드 구현 (새 문서만 수집)
- 문서 다운로드 기능
- LLM 통합 (CSS Selector 자동 추출)
- PostgreSQL 마이그레이션
