...
timeline
    ...
    2026-05-01 : Claude Code v2.1.126
              : claude project purge 명령어 추가
              : claude auth login 터미널 OAuth 코드 붙여넣기 지원
              : Windows PowerShell 7 감지 개선, 기본 셸 전환
              : allowManagedDomainsOnly 보안 버그 수정
    2026-05-04 : Claude Code v2.1.128
              : /color 랜덤 색상, /mcp 툴 수 표시
              : 플러그인 ZIP 아카이브 지원, vim Space 키 수정
              : 1M-context "Prompt is too long" 오류 수정
    2026-05-06 : Claude Code v2.1.129
              : --plugin-url 플래그, CLAUDE_CODE_FORCE_SYNC_OUTPUT
              : CLAUDE_CODE_PACKAGE_MANAGER_AUTO_UPDATE
              : Gateway 모델 발견 opt-in, skillOverrides 설정 정상화
    2026-05-06 : Claude Code v2.1.131
              : VS Code Windows 익스텐션 활성화 오류 수정
              : Mantle x-api-key 헤더 누락 수정
    2026-05-06 : Claude Code v2.1.132
              : CLAUDE_CODE_SESSION_ID 환경변수 (Bash 서브프로세스)
...
- **`claude project purge [path]`**: 특정 프로젝트의 모든 Claude Code 상태 삭제 (transcript, task, 파일 히스토리, config 항목)
  - `--dry-run`, `-y/--yes`, `-i/--interactive`, `--all` 플래그 지원
- **`/model` 피커 게이트웨이 연동**: `ANTHROPIC_BASE_URL`이 Anthropic 호환 게이트웨이를 가리킬 때 `/v1/models` 엔드포인트의 모델 목록 표시
- **`claude auth login` 터미널 OAuth**: 브라우저 콜백이 localhost에 도달할 수 없는 환경(WSL2, SSH, 컨테이너)에서 OAuth 코드를 터미널에 직접 붙여넣어 로그인 가능
- **Windows PowerShell 7 감지 개선**: Microsoft Store, MSI(PATH 미등록), .NET global tool 방식으로 설치된 PowerShell 7 자동 감지
- **Windows 기본 셸 변경**: PowerShell 활성화 시 Bash 대신 PowerShell을 기본 셸로 사용
- **Auto 모드 스피너 빨간색**: 권한 확인이 지연될 때 스피너가 빨간색으로 전환되어 시각적 피드백 제공
- **이미지 붙여넣기 자동 다운스케일**: 2000px 초과 이미지 붙여넣기 시 자동으로 축소

**보안 수정**

- **`allowManagedDomainsOnly` / `allowManagedReadPathsOnly` 무시 버그 수정**: 상위 우선순위 managed-settings 소스에 `sandbox` 블록이 없을 때 해당 설정이 적용되지 않던 문제 수정
- **`--dangerously-skip-permissions` 확장**: `.claude/`, `.git/`, `.vscode/`, 셸 설정 파일 등 보호 경로에 대한 쓰기 권한 프롬프트 우회 (파괴적 삭제 명령은 여전히 확인)

**버그 수정**

- 느린 연결/프록시 환경 및 IPv6 전용 devcontainer에서 OAuth 로그인 실패 수정
- Mac 슬립 후 재시작 시 "Stream idle timeout" 오류 수정
- 다수 UI, 렌더링, 플랫폼별 문제 수정
...
- **`/color` 랜덤 색상**: 인자 없이 `/color` 실행 시 랜덤 세션 색상 지정
- **`/mcp` 툴 수 표시**: 연결된 MCP 서버의 툴 개수 표시
- **플러그인 ZIP 아카이브**: `--plugin-dir`로 `.zip` 플러그인 아카이브 직접 지정 가능
- **`--channels` 콘솔 인증 지원**: `--channels` 플래그가 콘솔 인증과 함께 동작
- **`/model` 피커 개선**: Opus 항목 접혀서 표시 (공간 절약)
- **`EnterWorktree` 동작 변경**: `origin/<default-branch>` 대신 로컬 HEAD에서 브랜치 생성
- **서브프로세스 OTEL 격리**: 서브프로세스가 `OTEL_*` 환경변수를 더 이상 상속하지 않음
- **MCP `workspace` 예약 이름**: `workspace`는 MCP 서버의 예약 이름으로 지정
- **Reconnecting MCP 서버 요약**: 재연결 시 prefix별로 re-announced 툴 요약 표시
- **SDK 호스트 `localSettings` 제안**: SDK 호스트에 영속적 `localSettings` 제안 전달

**버그 수정**

- focus 모드에서 이전 응답이 dimming되던 문제 수정
- Kitty 터미널에서 `/exit` 시 스테이 데스크톱 알림 수정
- 드래그 앤 드롭 이미지 업로드 hang 수정
- 파이프 입력 10MB 초과 시 크래시 루프 수정
- 긴 URL 클릭 불가 문제 수정
- MCP 툴 결과에서 이미지 드롭 수정
- 1M-context 세션의 "Prompt is too long" 오류 수정
- vim 모드: `Space`가 커서 오른쪽으로 이동하도록 수정
- `/rename` 재개된 세션에서 실패 수정
- 1시간 프롬프트 캐시 TTL이 5분으로 다운그레이드되던 문제 수정
- Agent SDK가 잘못된 툴 이름으로 hang되던 문제 수정
...
- **`--plugin-url <url>` 플래그**: URL에서 플러그인 `.zip` 아카이브를 직접 가져오는 플래그 추가
- **`CLAUDE_CODE_FORCE_SYNC_OUTPUT=1`**: 동기화된 출력을 위한 환경변수 추가
- **`CLAUDE_CODE_PACKAGE_MANAGER_AUTO_UPDATE`**: Homebrew/WinGet에서 자동 업그레이드 제어 환경변수
- **Plugin 매니페스트 구조 변경**: `themes`와 `monitors`는 이제 `"experimental": { ... }` 아래에 위치해야 함
- **Gateway 모델 발견 opt-in**: `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1`로 opt-in (기본값 비활성화)
- **Ctrl+R 기록 피커**: 기본적으로 모든 프롬프트 표시 (v2.1.124 이전 동작 복원)
- **`skillOverrides` 설정 정상화**: `off`, `user-invocable-only`, `name-only` 옵션 정상 동작
- **`claude_code.pull_request.count` OTel 메트릭**: MCP 툴에서 생성된 PR/MR도 카운트
- **Policy 거부 오류에 API Request ID 포함**

**버그 수정**

- 인식 불가 400 상태 코드의 raw JSON 노출 수정
- `/clear` 후 터미널 탭 타이틀 미초기화 수정
- 권한 다이얼로그 중 세션 타이틀 칩 사라짐 수정
- 에이전트 패널이 서브에이전트 실행 중 숨겨지던 문제 수정
- 외부 에디터 핸드오프(Ctrl+G) 시 대화 히스토리 소실 수정
- `/context` 명령어가 ASCII 시각화를 대화에 덤프하던 문제 수정
- `Bash(mkdir *)`, `Bash(touch *)` 허용 규칙이 프로젝트 내 경로에 적용되지 않던 문제 수정
- `/clear` 후 VSCode에서 컨텍스트 초기화 수정
...
- **VS Code Windows 활성화 오류 수정**: 번들 SDK의 하드코딩된 빌드 경로(`createRequire` 폴리필 버그)로 인해 Windows에서 VS Code 익스텐션이 활성화되지 않던 문제 수정
- **Mantle 엔드포인트 인증 수정**: `x-api-key` 헤더 누락으로 Mantle 엔드포인트 인증이 실패하던 문제 수정
...
- **`CLAUDE_CODE_SESSION_ID` 환경변수**: Bash 툴 서브프로세스 환경에 세션 ID 자동 주입 (훅에 전달되는 `session_id`와 동일)
- **`CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1`**: 풀스크린 alternate-screen 렌더러 비활성화 → 터미널 네이티브 스크롤백 유지
- **이미지 붙여넣기 힌트**: Ctrl+V로 이미지 클립보드에서 읽는 동안 "Pasting…" 풋터 힌트 표시

**버그 수정**

- **외부 SIGINT 우아한 종료**: IDE 정지 버튼·`kill -INT`로 인한 외부 SIGINT가 graceful shutdown 실행하지 않던 문제 수정
- `--resume` 시 이모지 분할로 인한 `no low surrogate in string` 오류 수정
- `--permission-mode` 플래그가 plan-mode 세션 재개 시 무시되던 문제 수정
- 랩톱 슬립/웨이크 또는 Ctrl+Z/`fg` 후 풀스크린 모드 빈 화면 수정
- Indic conjunct 및 ZWJ 이모지에서 커서 중간 위치 수정
- Vim 연산자가 분해된(NFD) 악센트 문자를 손상시키던 문제 수정
- `/`로 시작하는 텍스트 붙여넣기가 입력을 삼키던 문제 수정
- Cursor/VS Code 1.92–1.104 마우스 휠 스크롤 속도 수정
- JetBrains IDE 2025.2 터미널 스크롤 수정
- `/usage` Ctrl+S의 Linux/X11 클립보드 hang 수정
- `/effort` 피커가 `CLAUDE_CODE_EFFORT_LEVEL` env var를 반영하지 않던 문제 수정
- 슬래시 커맨드 자동완성 팝업 3-5개로 제한되던 문제 수정
- statusline `context_window` 토큰 수가 누적 세션 토탈을 표시하던 문제 수정
- macOS에서 Alt+T(thinking 토글)가 "Option as Meta" 없이 작동하지 않던 문제 수정
- Windows에서 백그라운드 세션 재열기 후 키보드 입력 dead 문제 수정
- **stdio MCP 서버 무한 메모리 증가(10GB+ RSS) 수정**
- MCP 서버 `tools/list` 실패 시 조용히 0개 표시하던 문제 수정
- 미인증 claude.ai MCP 커넥터가 "failed" 대신 "needs auth" 표시하도록 수정
- Bedrock·Vertex에서 `ENABLE_PROMPT_CACHING_1H` 사용 시 400 오류 수정
